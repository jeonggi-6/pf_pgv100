...
#include <pf_pgv100/new_pgv_scan_data.h> // for message file
...
//////////////////////////////////////////////////
  ...
  ros::Publisher pgv100_pub = pub.advertise<pf_pgv100::new_pgv_scan_data>("new_pgv100_scan", 100); // second parameter is buffer
  ...
  while (ros::ok())
  {
    ...
    pf_pgv100::new_pgv_scan_data new_mesaj;
    ...
    new_mesaj.angle = agv_ang_des - cal_err_ang;          // degree
    new_mesaj.x_pos = (agv_x_pos_des - cal_err_x) / 10.0; // mm
    new_mesaj.y_pos = (agv_y_pos_des - cal_err_y) / 10.0; // mm

    // pgv100_pub.publish(mesaj);
    pgv100_pub.publish(new_mesaj);
